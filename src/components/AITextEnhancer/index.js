...
class AITextEnhancer extends HTMLElement {
  ...
  get currentContent() {
    if (this.editorAdapter && this.editorId) {
  ...
  async handleToolAction(event) {
    ...
    try {
      ...
      if (!content) {
        throw new Error("No content available to enhance. Please add some content to the editor.");
      }
      ...
      const completeText = await this.apiClient.enhance(content, action, context);
